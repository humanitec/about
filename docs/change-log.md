...
- **Improved:** There is no longer a limit of the number of Applications users can create via UI.

- **Fixed:** The [Profile Settings](https://docs.humanitec.com/reference/user-interface/top-bar#user-menu) page now correctly lists users' [roles](https://docs.humanitec.com/guides/manage-users-permissions/role-based-access-control) in all Organisations they belong to. 

- **Fixed:** Resolved an issue where Placeholders autocompletion would not work properly with inputws typed manually and not selected from the dropdown. 
